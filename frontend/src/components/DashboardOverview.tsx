...
import { FiFileText, FiCheckSquare, FiTarget, FiClock, FiPlus, FiChevronRight } from "react-icons/fi";
import Link from "react-router-dom"; // Actually, next/link should be used. Wait, I'm at the frontend.
...
export default function DashboardOverview() {
  // Mock data for now
  const stats = [
    { icon: FiFileText, label: "Total Notes", value: 124, trend: "+12", color: "text-brand-primary", bg: "bg-brand-primary/10" },
    { icon: FiCheckSquare, label: "Active Tasks", value: 8, trend: "3 due today", color: "text-brand-secondary", bg: "bg-brand-secondary/10" },
    { icon: FiTarget, label: "Goals Progress", value: "65%", trend: "2/3 active", color: "text-green-400", bg: "bg-green-400/10" },
    { icon: FiClock, label: "Today's Focus", value: "4h 20m", trend: "+15%", color: "text-yellow-400", bg: "bg-yellow-400/10" },
  ];

  const recentNotes = [
    { title: "Next.js 15 Implementation Notes", date: "2 mins ago", tags: ["dev", "learning"] },
    { title: "Personal Branding Strategy", date: "1 hour ago", tags: ["career", "ideas"] },
    { title: "PK-Manager System Architecture", date: "Yesterday", tags: ["project", "arch"] },
  ];
  ...
  return (
    <div className="flex flex-col gap-8 max-w-6xl mx-auto py-8 px-4">
      ...
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-3xl font-display font-bold text-text-main mb-2">Welcome back, Anthony</h1>
          <p className="text-text-muted">Here's what's happening in your second brain today.</p>
        </div>
        <NextLink href="/notes/new" className="flex items-center gap-2 bg-brand-primary hover:bg-brand-primary/90 text-white font-bold px-5 py-2.5 rounded-xl shadow-lg shadow-brand-primary/25 transition-all active:scale-[0.98]">
          ...
          <div className="bg-surface-soft border border-white/5 rounded-3xl p-6">
            ...
            <div className="flex flex-col gap-3">
              {recentNotes.map((note) => (
                <div key={note.title} className="p-4 rounded-2xl bg-surface-base border border-white/5 hover:border-white/10 transition-all flex items-center justify-between group cursor-pointer">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-text-main group-hover:text-brand-primary transition-colors">{note.title}</h4>
            ...
            <div className="flex flex-col gap-4">
              {[
                ...
                <div key={task.label} className="flex items-center gap-3 group">
                  ...
                  <span className="text-sm text-text-main font-medium">{task.label}</span>
                </div>
              ))}
              <NextLink href="/tasks" className="mt-2 text-xs font-bold text-text-muted hover:text-text-main transition-colors text-center py-2 border border-dashed border-white/10 rounded-xl">
                + Add Task
              </NextLink>
            </div>
          </div>
