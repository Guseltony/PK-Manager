  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 px-4 py-6 md:px-6">
      <section className="grid gap-6 xl:grid-cols-[1.5fr_1fr]">
        <div className="rounded-[28px] border border-white/10 bg-surface-soft p-6 shadow-2xl">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-7 text-text-muted">
                Drop raw thoughts here and let the routing engine classify, enrich, and move them into the right PKM system.
              </p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-right">
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-text-muted">Queue Health</p>
              <p className="mt-2 text-2xl font-black text-white">{queue.length}</p>
              <p className="text-xs text-text-muted">awaiting or needing attention</p>
            </div>
          </div>
          ...
          <div className="mt-6 rounded-[24px] border border-white/10 bg-black/20 p-4">
            ...
            <div className="mt-4 flex flex-col gap-3 border-t border-white/5 pt-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-xs text-text-muted">
                Routing is automatic. Original input is preserved in the audit trail.
              </p>
              <button
                type="button"
        ...
        <div className="rounded-[28px] border border-white/10 bg-surface-soft p-6">
          ...
          {selectedItem ? (
            <div className="mt-4 space-y-4">
              <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-bold text-white">{selectedItem.title || "Untitled entry"}</p>
                    <p className="mt-1 text-xs text-text-muted">
                      {selectedItem.type || "PROCESSING"} • {Math.round((selectedItem.confidence || 0) * 100)}% confidence
                    </p>
                  </div>
                  <span className={`rounded-full border px-2 py-1 text-[10px] font-black uppercase tracking-[0.18em] ${typeColorMap[selectedItem.type || ""] || "border-white/10 bg-white/5 text-text-muted"}`}>
                ...
                {selectedItem.suggestedActions?.length ? (
                  <div className="mt-4 space-y-2">
                    {selectedItem.suggestedActions.map((action) => (
                      <div key={action} className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-text-main">
                        {action}
                      </div>
                    ))}
                  </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                <p className="text-[10px] font-black uppercase tracking-[0.18em] text-text-muted">Related Signals</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {(selectedItem.links?.dreams || []).map((label) => (
                    <span key={`dream-${label}`} className="rounded-full border border-orange-400/20 bg-orange-400/10 px-2 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-orange-200">
                      Dream: {label}
                    </span>
                  ))}
                  {(selectedItem.links?.tasks || []).map((label) => (
                    <span key={`task-${label}`} className="rounded-full border border-amber-400/20 bg-amber-400/10 px-2 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-amber-200">
                      Task: {label}
                    </span>
                  ))}
                  {(selectedItem.links?.notes || []).map((label) => (
                    <span key={`note-${label}`} className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-2 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-emerald-200">
                      Note: {label}
                    </span>
                  ))}
                  {(selectedItem.links?.ideas || []).map((label) => (
                    <span key={`idea-${label}`} className="rounded-full border border-sky-400/20 bg-sky-400/10 px-2 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-sky-200">
                      Idea: {label}
                    </span>
                  ))}
                </div>
            ...
            <div className="mt-4 rounded-2xl border border-dashed border-white/10 bg-black/20 p-6 text-sm text-text-muted">
              Capture your first entry to see routing suggestions, related entities, and audit detail.
            </div>
          )}
        </div>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.05fr_1.25fr]">
        <div className="rounded-[28px] border border-white/10 bg-surface-soft p-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-brand-primary">Unprocessed Queue</p>
              <p className="mt-2 text-sm text-text-muted">Failed and in-flight captures stay visible until they are resolved.</p>
            </div>
        ...
        <div className="rounded-[28px] border border-white/10 bg-surface-soft p-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-brand-primary">Processed History</p>
              <p className="mt-2 text-sm text-text-muted">Every routed item keeps its original wording, route target, and suggestions.</p>
            </div>
            <span className="rounded-full border border-white/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-text-main">
            ...
            {isLoading ? Array.from({ length: 4 }).map((_, index) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-base font-bold text-white">{item.title || item.rawInput.slice(0, 80)}</p>
                    <p className="mt-1 text-xs text-text-muted">{dayjs(item.createdAt).fromNow()} • routed to {item.routedEntityType || "system"}</p>
                  </div>
                ...
                <p className="mt-3 line-clamp-2 text-sm leading-6 text-text-muted">{item.rawInput}</p>

                <div className="mt-4 flex flex-wrap gap-2">
                  {(item.tags || []).map((tag) => (
                    <span key={tag} className="rounded-full border border-white/10 bg-white/5 px-2 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-text-main">
                      #{tag}
                    </span>
                  ))}
                </div>
