...
import { FiActivity, FiBell, FiBrain, FiLoader, FiRefreshCcw, FiSave, FiSettings2, FiShield, FiTarget, FiZap } from "react-icons/fi";
    ...
    <div className="mx-auto max-w-7xl space-y-8 px-4 py-6 md:px-6">
        ...
        <div className="bg-linear-to-r from-brand-primary/18 via-transparent to-emerald-400/15 px-6 py-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              <p className="text-xs font-black uppercase tracking-[0.24em] text-brand-primary">Cognitive Operating System Control</p>
              <h1 className="mt-3 text-4xl font-black tracking-tight text-white md:text-5xl">Settings</h1>
              <p className="mt-3 text-sm leading-7 text-text-muted">
                These settings are the highest authority layer in PK-Manager. They define how AI behaves, how strict accountability becomes, and where automation is allowed to operate.
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              <ActionButton
                ...
                onClick={async () => {
                  ...
                  setLocalChanges({});
                  if (next) {
                    setSaveMessage("System behavior profile reset to defaults.");
                    setTimeout(() => setSaveMessage(""), 2500);
                  }
                }}
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <SettingsCard
          ...
          icon={FiBrain}
        >
          <SelectField label="AI strictness" value={draft.aiStrictness} options={groupedSelects.aiStrictness!} onChange={(value) => handleSelect("aiStrictness", value as Settings["aiStrictness"])} />
          <SelectField label="AI proactiveness" value={draft.aiProactiveness} options={groupedSelects.aiProactiveness!} onChange={(value) => handleSelect("aiProactiveness", value as Settings["aiProactiveness"])} />
          <SelectField label="AI reasoning depth" value={draft.aiReasoningDepth} options={groupedSelects.aiReasoningDepth!} onChange={(value) => handleSelect("aiReasoningDepth", value as Settings["aiReasoningDepth"])} />
        </SettingsCard>
        ...
        <SettingsCard
          ...
          icon={FiZap}
        >
          <ToggleField label="Auto-task generation from dreams" checked={draft.autoTaskGenerationFromDreams} onToggle={() => handleToggle("autoTaskGenerationFromDreams")} />
          <ToggleField label="Auto-linking in knowledge graph" checked={draft.autoLinkingKnowledgeGraph} onToggle={() => handleToggle("autoLinkingKnowledgeGraph")} />
          <SelectField label="Auto-insight frequency" value={draft.autoInsightFrequency} options={groupedSelects.autoInsightFrequency!} onChange={(value) => handleSelect("autoInsightFrequency", value as Settings["autoInsightFrequency"])} />
          <SelectField label="Inbox routing sensitivity" value={draft.inboxRoutingSensitivity} options={groupedSelects.inboxRoutingSensitivity!} onChange={(value) => handleSelect("inboxRoutingSensitivity", value as Settings["inboxRoutingSensitivity"])} />
        </SettingsCard>
        ...
        <SettingsCard
          ...
          icon={FiActivity}
        >
          <SelectField label="Task prioritization mode" value={draft.taskPrioritizationMode} options={groupedSelects.taskPrioritizationMode!} onChange={(value) => handleSelect("taskPrioritizationMode", value as Settings["taskPrioritizationMode"])} />
          <SelectField label="Deadline enforcement" value={draft.deadlineEnforcement} options={groupedSelects.deadlineEnforcement!} onChange={(value) => handleSelect("deadlineEnforcement", value as Settings["deadlineEnforcement"])} />
        </SettingsCard>
        ...
        <SettingsCard
          ...
          icon={FiShield}
        >
          <SelectField label="Ledger strictness" value={draft.ledgerStrictness} options={groupedSelects.ledgerStrictness!} onChange={(value) => handleSelect("ledgerStrictness", value as Settings["ledgerStrictness"])} />
          <SelectField label="Failure visibility" value={draft.failureVisibility} options={groupedSelects.failureVisibility!} onChange={(value) => handleSelect("failureVisibility", value as Settings["failureVisibility"])} />
        </SettingsCard>
        ...
        <SettingsCard
          ...
          icon={FiTarget}
        >
          <ToggleField label="Auto project generation from dreams" checked={draft.autoProjectGeneration} onToggle={() => handleToggle("autoProjectGeneration")} />
          <ToggleField label="Auto task breakdown from dreams" checked={draft.autoTaskBreakdownFromDreams} onToggle={() => handleToggle("autoTaskBreakdownFromDreams")} />
          <SelectField label="Dream progress sensitivity" value={draft.dreamProgressSensitivity} options={groupedSelects.dreamProgressSensitivity!} onChange={(value) => handleSelect("dreamProgressSensitivity", value as Settings["dreamProgressSensitivity"])} />
        </SettingsCard>
        ...
        <SettingsCard
          ...
          icon={FiBell}
        >
          <ToggleField label="Task reminders" checked={draft.taskReminders} onToggle={() => handleToggle("taskReminders")} />
          <ToggleField label="Focus session alerts" checked={draft.focusSessionAlerts} onToggle={() => handleToggle("focusSessionAlerts")} />
          <ToggleField label="Daily insight summaries" checked={draft.dailyInsightSummaries} onToggle={() => handleToggle("dailyInsightSummaries")} />
          <ToggleField label="Missed journal reminders" checked={draft.missedJournalReminders} onToggle={() => handleToggle("missedJournalReminders")} />
        </SettingsCard>
      </div>
      ...
      <section className="rounded-[28px] border border-white/10 bg-surface-soft p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-black uppercase tracking-[0.2em] text-brand-primary">Authority Rule</p>
            <h2 className="mt-1 text-xl font-black text-white">Settings always win</h2>
          </div>
        </div>
        <p className="mt-4 max-w-4xl text-sm leading-7 text-text-muted">
          If an AI suggestion conflicts with these settings, this control profile is the final authority. That makes this page the behavior contract for every automation and intelligence layer across inbox, tasks, dreams, ledger, and insights.
        </p>
      </section>
    </div>
  );
