...
import { FiActivity, FiClock, FiDownload, FiList } from "react-icons/fi";
...
export default function LedgerDashboard() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-surface-base">
      <div className="p-4 sm:p-6 lg:p-8 pb-4 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/5">
        <div>
          ...
          <p className="text-text-muted text-sm max-w-lg leading-relaxed">
            Immutable execution engine. Analyze your productivity habits and
            historical performance.
          </p>
        </div>

        <div className="flex items-center gap-2">
          <div className="flex overflow-x-auto custom-scrollbar bg-white/5 p-1 rounded-2xl border border-white/5 w-full md:w-auto">
            <button
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 custom-scrollbar">
                ...
                <tbody className="divide-y divide-white/5">
                  {logs.map((log) => (
                    ...
                    >
                      ...
                      <td className="p-4">
                        <span
                          className={`text-[10px] px-2 py-1 rounded-full border uppercase tracking-widest font-black ${
                            log.priority === "urgent"
                      ...
                      <td className="p-4">
                        <div className="flex gap-1 flex-wrap">
                          {log.tags.map((tag) => (
                            <span
                              key={tag}
                              className="text-[10px] uppercase tracking-wider px-2 py-1 bg-white/5 text-text-muted rounded-md font-medium"
                            >
                              #{tag}
                            </span>
                          ))}
                      ...
                      <td className="p-4 text-sm text-text-muted">
                        {log.note ? (
                          <span className="hover:text-brand-primary cursor-pointer line-clamp-1">
                            {log.note.title}
                          </span>
                        ) : (
                          "-"
        ...
        {view === "heatmap" && (
          <div className="glass p-8 rounded-[2rem] border border-white/5 flex flex-col items-center justify-center min-h-100">
            <h2 className="text-xl font-bold font-display text-text-main mb-6">
              Activity Heatmap (V3 Concept)
            </h2>
              ...
              {summaries.map((sum) => (
                <div
                  ...
                  className={`w-10 h-10 rounded-lg cursor-help transition-all hover:scale-110 ${
                    sum.productivityScore && sum.productivityScore > 80
                      ? "bg-amber-500 shadow-lg shadow-amber-500/20"
                      : sum.productivityScore && sum.productivityScore > 50
                        ? "bg-emerald-500/60"
                        : sum.productivityScore && sum.productivityScore > 20
                          ? "bg-emerald-500/30"
                          : "bg-white/5"
                  }`}
                />
        ...
        {view === "replay" && (
          <div className="glass p-8 rounded-[2rem] border border-white/5 flex flex-col items-center justify-center min-h-100">
            ...
            <p className="text-text-muted max-w-sm text-center">
              The replay engine is currently calibrating. Chronological sequence
              reconstruction will be available in the next major patch.
            </p>
          </div>
        )}
      </div>
